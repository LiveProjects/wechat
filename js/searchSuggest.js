...
$(function(){
	//监听文本框输入变化
	$("#suggest_input").bind('input propertychange',function(){
		//创建ajax对象函数
...
$(function(){
	//按下按键后300毫秒显示下拉提示
	$("#suggest_input").keyup(function(){/*$("#suggest_form").submit();*/
		setInterval(changehover,300);
		function changehover(){
			$("#suggest_ul li").hover(function(){ $(this).css("background","#eee");},function(){ $(this).css("background","#D2E9FF");});
			
			$("#suggest_ul li").click(function(e){ 
				e.stopPropagation();
				$("#suggest_input").val($(this).text());
				if($(this).text()=="没有此商品，请重新输入关键字"){
					//alert(123);
				}
			});
			$("#suggest_ul li").click(function(e){ 
